	...
	class Master_list_model extends CI_Model
	{
		...
		{			
			...
			$query = $this->db->query("SELECT COUNT(household_id) as fever_house
						FROM 
							(SELECT MAX(imcase_no), person_id, imcase_no, MAX(created_on) as created_on
		...
		{
			$this->db->from('immediate_cases');
			$where = "imcase_no = (SELECT MAX(imcase_no) as created_on FROM immediate_cases WHERE person_id = '" . $person_id . "' AND DATEDIFF(NOW(), created_on) <= '7'
